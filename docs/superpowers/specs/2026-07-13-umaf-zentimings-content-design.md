...
7. UMAF：高风险固件设置入口；用于进入 AMD PBS/CBS/Overclocking 隐藏菜单，修改包括内存频率和时序在内的真实固件参数。卡片内高亮显示“本机约 80MB 的 UMAF 启动分区不可删除或格式化”，说明错误设置可能导致无法启动，并保留开机持续按 F2 的当前进入路径；详细安装和进入方法指向 03 的教程 01。

UMAF 分区警告不回到 Hero。02 软件部分新增醒目提醒，让接手者尽早看到；页尾 UMAF 附录的原位置同时保留完整提醒和 F2 进入路径，供真正操作时再次对照。

02 的 UMAF 红色分区警告与下一行“进入：……”之间保留明显的段落间距，警告块下边距为 `1.5rem`，不得紧贴。
...
- UMAF 区块说明它是填写回退值的固件入口；两张截图主要用于定位 DDR SPD Timing 与 DDR Non-SPD Timing 字段，真正回退值使用 ZenTimings 当前稳定十进制读数换算后的明确十六进制列表。

在 UMAF 两张截图之前增加“回退十六进制值”说明：以 ZenTimings 当前稳定十进制读数为基准换算，只有 tWR 例外，ZenTimings 显示 66（通常换算为 `42`），回退时仍填写用户确认的 `40`。列表标题已标明十六进制，因此每个值不加 `0x` 前缀；小于 10H 的值保留两位写法。
...
- 截图中其余 `Auto` 字段继续保持 `Auto`。

旧 UMAF 图中有些值与 ZenTimings 当前稳定值不一致，因此图片用于找位置，本列表用于回退填写；填写并重启后，再用 ZenTimings 核对实际生效值。
...
原 04 的两条 B 站教程整体移动到 03 前部，精确顺序为“已完成前置设置摘要 → 教程 01 → 教程 02 → 当前设置截图”：教程 01 紧邻 CCD1 / BIOS / UMAF 状态，教程 02 紧邻 CPU 睿频 / 高性能电源计划状态。教程用于需要重新安装、重新进入或回看操作步骤时查阅，不表示现在需要重复执行。
...
- 两条 B 站教程都位于 03 的前置设置摘要之后、第一张设置截图之前，04 不再包含教程；
- ZenTimings 文案包含“只读”“当前实际生效”“不能修改时序”“不能替代稳定性测试”的语义；
- UMAF 文案包含 AMD PBS/CBS/Overclocking、真实固件参数、约 80MB 分区不可删除或格式化，以及错误设置可能无法启动；
- 约 80MB 警告同时出现在 02 软件区和页尾 UMAF 附录，Hero 不出现；
- “持续按 F2”的进入路径同时位于 02 和页尾附录；
- 页尾附录不存在两个旧等号标题，但仍包含三张内存截图与局部回退边界；
- 页尾 UMAF 图片前包含从 ZenTimings 当前稳定值换算出的完整十六进制回退列表、tWR `40` 例外和 `Auto` 保持规则；
- 构建、HTML validator、PC 2x 和移动端 3x 截图全部通过。

## 01 温度文案补充

温度卡片的对象必须明确为 CPU：标签使用“游戏时 CPU 温度”，正文使用“使用上风压散热器后，游戏时 CPU 温度稳定在 85–87°C 可接受。”，不再使用含糊的“最终游戏温度”。

Hero 的内存时序卡同时给出调整前后：数值显示 `C36 · 5600`，正文写明“默认 C42 / 5200 MT/s → 当前 C36 / 5600 MT/s；已完成超频和时序收紧，内存时序已调整并稳定运行。”

页尾 ZenTimings 稳定结果也写明“内存从默认 C42 / 5200 MT/s 调整为当前 C36 / 5600 MT/s，并稳定运行。”，作为比首屏更完整的结果留档。
