...
同一组参数随后已在本机用 CLI 真实验证：`tcb app deploy laptop --cwd ./dist --framework static --output-dir . --deploy-path / --force` 成功为原应用创建 `laptop-010`，线上 `index.html` 与本地 `dist/index.html` 字节和 SHA-256 完全一致。这证明 CLI 的应用目标语义正确，GitHub Actions 的旧失败只来自 runner 到 COS 的上传链路；CLI 因此保留为本地人工回退，不再作为 GitHub-hosted runner 的生产传输方式。
...
2. `npm ci`；
3. `npm test`，在 GitHub 侧只执行源码与部署契约测试；
4. 使用固定版本腾讯云官方 TCB API SDK 查询现有 `laptop`；
5. 在 checkout 精确等于 `github.sha` 且 tracked files 无改动时计算源码摘要，然后通过 `BuildType=GIT` 创建该应用的新版本；
6. CloudBase 服务端从公开 GitHub 仓库拉取 `master`，执行 `npm ci`，并在构建前校验源码摘要；
7. CloudBase 只执行一次正式的 `npm run build`，随后独立校验生成的 `dist/index.html`，并再次确认构建没有改写源码；
8. CloudBase 把本次临时生成的 `dist/` 发布到 `/`；
9. Actions 按 `CreateCloudApp` 返回的 `BuildId` 轮询，只有新版本状态为 `SUCCESS` 才成功结束。

GitHub Actions 是测试门禁，CloudBase 是唯一正式构建位置。`dist/` 写入 `.gitignore` 且不受 Git 跟踪，SDK 部署入口会要求 checkout 精确等于 `github.sha`、tracked files 无改动，再计算完整源码摘要。CloudBase 拉取相同快照后生成并发布 `dist/`，因此既没有 GitHub 与 CloudBase 的重复构建，也不再从 GitHub runner 上传 ZIP。
...
```js
{
  ...
  staticConfig: {
    ...
    staticEnv: {
      variables: [
        { key: 'EXPECTED_GITHUB_SHA', value: '<github.sha>' },
        { key: 'EXPECTED_SOURCE_SHA256', value: '<tracked-files-sha256>' },
      ],
    },
  },
}
...
控制台只提供 `codeBranch`，没有 commit SHA 字段。Actions 在 `HEAD` 精确等于 `github.sha` 且无 tracked diff 的 checkout 上，对全部 Git 跟踪源码的路径和内容计算确定性 SHA-256；部署请求把当前 `github.sha` 和该摘要写入两个非敏感构建变量。CloudBase 在 `npm ci` 后运行白名单允许的 `npm run verify:deploy`，由 npm script 调用 `node scripts/verify-deploy-revision.mjs`：先遍历实际构建目录并重算摘要，再执行唯一一次正式构建和独立 HTML 校验，最后重新计算摘要以确认构建没有改写源码。CloudBase 的构建目录不保留 `.git`，所以不能依赖 `git rev-parse HEAD`，仅查询远端分支头也不能证明已经克隆的内容；源码摘要直接绑定 Actions checkout 与 CloudBase 工作区，避免这两个缺口。摘要会排除 `.git`、`node_modules`、`dist`、构建缓存和本地工具状态，但任何其他额外、缺失或变更文件都会安全失败。CloudBase 也会拒绝直接以 `node` 开头的自定义构建命令，且不能把 `$(...)` 或 `&&` 写入命令，因此必须通过 npm script 间接执行且避免 shell 拼接。为避免两个远端部署交叠，Actions 的生产并发组必须串行排队，不再取消正在运行且可能已经触发 CloudBase 版本创建的 job。每次脚本只轮询自己创建后返回的 `BuildId`，不能用“最新版本”代替。
...
workflow 删除全局 CLI 安装、`tcb login`、ZIP 上传和 `working-directory: dist`。三个 Secrets 只注入受 `master` guard 保护的最后一个 SDK 部署步骤。PR 执行安装与测试，不生成正式产物。
...
真实验收在推送后完成：目标 Actions revision 成功、无 annotations、`laptop` 路径仍为 `/`、状态为 `SUCCESS`、版本不再是 `laptop-010`，线上 `index.html` 与该 revision 在本地重建的产物一致，Actions 日志不含三个 Secret 原值。
