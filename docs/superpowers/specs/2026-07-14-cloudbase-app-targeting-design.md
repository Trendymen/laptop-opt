...
```js
{
  ...
  staticConfig: {
    ...
    staticCmd: {
      installCmd: 'npm ci',
      buildCmd: 'test "$(git rev-parse HEAD)" = "$EXPECTED_GITHUB_SHA" && npm run verify',
...
控制台只提供 `codeBranch`，没有 commit SHA 字段。部署请求把当前 `github.sha` 写入非敏感构建变量，CloudBase 在发布前用 `git rev-parse HEAD` 比对；分支若在克隆前移动会安全失败，避免发布错误 revision。为避免两个远端构建交叠，Actions 的生产并发组必须串行排队，不再取消正在运行且可能已经触发 CloudBase 构建的 job。每次脚本只轮询自己创建后返回的 `BuildId`，不能用“最新版本”代替。
