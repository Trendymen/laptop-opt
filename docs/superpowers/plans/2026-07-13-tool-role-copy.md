...
- The machine-specific `约 80MB` UMAF startup partition warning appears exactly once in section 02, not in the Hero or final appendix.
...
Move both existing `<article class="tutorial">` elements unchanged from section 04 to the end of section 03. Keep both exact titles, BV numbers, descriptions, timestamps, URLs, `target`, and `rel` attributes. Section 04 must then contain only its heading and the existing UXTU recovery `.media-step`.
...
Replace the UMAF appendix header and remove its old warning paragraph:
...
```css
.umaf-reference header p { color: var(--muted); }
...
.tools-list .tool-warning { margin-top: 1.25rem; padding: 1rem; color: var(--text); background: #26161b; }
```
