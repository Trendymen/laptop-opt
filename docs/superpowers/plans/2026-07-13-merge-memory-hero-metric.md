...
- “内存通道”卡正文固定为“原装 32GB 英睿达镁光单条已更换为十铨 16GB × 2（海力士 M-die）双内存条，组成 32GB 双通道。”
...
- [ ] **Step 1: 写失败测试**
  ...
  在内容测试中断言 Hero 恰有五张指标卡，且仅有一张标题为“内存超频与时序”的卡；主值和说明使用全新合并文案。“内存通道”卡必须记录英睿达镁光 32GB 单条更换为十铨 16GB × 2（海力士 M-die）及组成 32GB 双通道。在布局测试中断言 `.hero-metric--memory` 默认 `grid-column: span 2`，600px 以下为 `grid-column: auto`。
